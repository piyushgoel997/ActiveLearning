...
from Evaluation import calc_accuracy, roc, auc, compare_posterirors, f1_score
...
def active_learning(X, Y, X_true, Y_true, true, learner, sampling=random_sampling, iters=100, metric=sse,
    ...
    for j in range(iters):
        ...
        if len(indices) == prev_len:
            break
        del models
    print("Final accuracy", acc[-1], ", Final F1-score", f1scores[-1], ", Final AUC", aucs[-1],
          ", Final diff between the learned and the actual posterior", comparison[-1])
    return np.array(acc), np.array(f1scores), np.array(aucs), np.array(comparison)
...
def run(X, Y, true, learner, sampling_methods, num_committees, name="", num_exp=10, iters=100):
    ...
    for i in range(len(sampling_methods)):
        ...
        f1_scores.append(np.zeros((iters,)))
    ...
    for i in range(num_exp):
        ...
        for j, (a, f, ra, pd) in enumerate(exp_result):
            accuracy[j] += np.array(extend_array(a, iters))
            f1_scores[j] += np.array(extend_array(f, iters))
    ...
    plot(f1_scores, name, "F1-scores", fn_names)
...
if __name__ == "__main__":
    ...
    if args.gendata:
        ...
        mu0, covar0, = np.array([0, 0]), np.array([1, 1.5]) * np.identity(2)
        mu1, covar1 = np.array([1, 2]), np.array([1, 2]) * np.identity(2)

        X, Y = normal_data_generator(1, 20_000, mu0, covar0, mu1, covar1)
    ...
    run(X, Y, true, [nn, nn_predict],
        ...
        name=args.loc, num_exp=30, iters=250)
